...
<#
.SYNOPSIS
    Checks if a certificate file meets all the requirements for the vCenter Server certificate (5.5 or later).
...
.EXAMPLE
    ...
    Checks if the certificate located in the default path on the local vCenter Server meets all the requirements for vCenter 5.5 or 6.0.

.EXAMPLE
    Test-VpxdCertificate -CertFilePath $env:USERPROFILE\Desktop\rui.crt -vCenterServerFQDN "VC.vcloud.local" -Verbose

    Checks if the certificate file on the user's desktop meets all the requirements for vCenter 5.5 or 6.0, displaying verbose information.
...
[cmdletbinding()]
    ...
    Process {
        ...
        $RequiredKeyUsageFlags = [Security.Cryptography.X509Certificates.X509KeyUsageFlags]"KeyEncipherment, DigitalSignature, DataEncipherment"
        Write-Verbose "Required key usages : KeyEncipherment, DigitalSignature, DataEncipherment"
            
        $KeyUsagesTest = $KeyUsageExtension.KeyUsages -eq $RequiredKeyUsageFlags
        ...
        Else {
            # Properties of the custom output object
